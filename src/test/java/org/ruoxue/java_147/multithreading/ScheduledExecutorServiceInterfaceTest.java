...
public class ScheduledExecutorServiceInterfaceTest {
	...
	public void cancelFalse() {
		...
		if (false == futureA.isDone()) {
			boolean cancel = futureA.cancel(false);
			System.out.println(String.format("%s T[%d] %d task: A, cancel: %b, done: %b", df.format(new Date()),
					Thread.currentThread().getId(), System.currentTimeMillis() - start, cancel,futureA.isDone()));
		}
	...
	public void cancelTrue() {
		...
		if (false == futureA.isDone()) {
			boolean cancel = futureA.cancel(true);
			System.out.println(String.format("%s T[%d] %d task: A, cancel: %b", df.format(new Date()),
